      ...
      <div class="row">
        ...
        <tbody>
          <tr *ngFor="let sec of section_data; let i = index" style="">
            ...
            <td>
            	<a class="waves-effect waves-light" routerLink="section/{{sec.id}}"><i class="material-icons left">announcement</i></a>
...
section_data;
  ...
  onSubmit(sectionForm: NgForm) {
    ...
    if(sectionForm.valid){
      this.formValid = true;
      this.adminService.addSection(sectionForm.value).subscribe(res => {
        if(res['success']){
          this.sec = []
  ...
  onDelete(id){
    Materialize.Toast.removeAll();
    this.adminService.deleteSection(id).subscribe(res => {
      if(res['success']){
        this.sec = []
